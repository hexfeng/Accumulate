...
def make_client(tmp_path, http: FakeSimpleFinHttp | None = None) -> tuple[TestClient, SimpleFinCredentialStore, FakeSimpleFinHttp]:
    ...
    return TestClient(create_app(store=LocalStore(), simplefin_service=service)), credential_store, fake_http
...
def simplefin_get_windows(http: FakeSimpleFinHttp) -> list[dict[str, list[str]]]:
    return [parse_qs(urlsplit(url).query) for method, url, _ in http.requests if method == "GET"]
...
def test_simplefin_connect_claims_setup_token_and_stores_access_url_securely(tmp_path):
    ...
    assert {window["version"][0] for window in windows} == {"2"}
    assert {window["pending"][0] for window in windows} == {"1"}
    for window in windows:
        assert int(window["end-date"][0]) - int(window["start-date"][0]) <= 45 * 24 * 60 * 60
...
def test_simplefin_sync_fetches_accounts_and_transactions_without_changing_ui_contract(tmp_path):
    ...
    client.app.state.store.record_simplefin_transaction_coverage("2025-06-11", "2026-06-11", FIXED_NOW)
    ...
    assert http.requests == [
        (
            "GET",
            "https://bridge.example.test/accounts-token/accounts?version=2&start-date=1777291200&end-date=1781179200&pending=1",
            {
...
def test_simplefin_connect_backfills_transactions_and_records_local_coverage(tmp_path):
    client, _, http = make_client(tmp_path)
    ...
    assert body["transaction_coverage_end"] == "2026-06-11"
    assert len(simplefin_get_windows(http)) == 9
    assert client.get("/api/transactions").json()[0]["external_id"] == "txn-1"
...
def test_simplefin_sync_imports_transactions_when_bridge_returns_range_warning(tmp_path):
    http = FakeSimpleFinHttp()
    http.accounts_response.body["errlist"] = [
        {
            "code": "gen.api",
